...
export default function Dashboard({navigation}){
  ...
  const calldata = async() => {
    ...
    const query = `SELECT category, date, [desc], idx, thumbnail, Media1, title, name, Like_cnt, Reply_cnt, View_cnt, email FROM bbs WHERE Reply_chk=0 ORDER BY idx desc`
  ...
  const upView = async(content) => {
    ...
    if (glovar.detailViewState == false) {
      ...
      navigation.navigate('DetailPage', {item: content}/*, glovar.lastDetail = content*/)
      // glovar.P_idx = content.idx
  ...
  return ready ? <Loading/> : (
    <SafeAreaView style={styles.container}>
      ...
      <TouchableOpacity style={styles.button} onPress={()=>{navigation.navigate('PostPage')}}>

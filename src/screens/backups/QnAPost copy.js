...
const PostPage = ({ navigation }) => {
  ...
  const writedone = async() => {
    if (title.value === "") {
      ...
      const query1 = `INSERT INTO QnA (category, title, [desc], email, name) VALUES('${String(category)}', '${String(title)}', '${String(desc)}', '${String(glovar.logininfo.email)}', '${String(glovar.logininfo.name)}')`
      ...
      navigation.navigate('QnADetail', {item: glovar.lastQnADetail[0]})
      glovar.P_idx = glovar.lastQnADetail[0].idx

...
const PostPage = ({ navigation }) => {
  ...
  const extrectURIs = (val) => {
    val.map((content, i) => {
      const uri = content.path.split('file://')[1]
      // setUris([...uris, uri])

      // const a = [1,2,3,4,5];
      // const b = a.map(1 => 1 * 2);

      // setUris(uris.concat(content.path))
      setUris(uris.concat(uri))

      // console.log("extrectURIs_map() ========================================");
      // console.log("content.path");
      // console.log(uri);
      // console.log("uris");
      // console.log(uris);
      // console.log("content");
      // console.log(content);    // 4개가 각각 나옴 uri만 뽑아서 배열로 저장할 것임
      // console.log("uris");
      // console.log(uris);    // 4개 이미지를 선택했는데 마지막 사진인지 처음 사진인지 하나의 이미지 uri만 4번 나옴
      // console.log("==================================================");
    })
    ...
    console.log(uris);
    console.log("==================================================");
    // 배열 0, 1, 2 ... 에서 차례로 선택해서 uri만 뽑아내야 함
    // 그냥 나중에 .uri 하면 되는거 아닌가?
    // react-native-image-zoom-viewer에서 uri 배열만 쓸 것이므로 uri만 따와서 배열로 만들어야 함
    // [0].uri[1].uri[2].uri[3].uri[4].uri 이런 식으로 
  }

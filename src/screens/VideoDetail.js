...
export default function VideoDetail({navigation, route}) {
    ...
    const callreply = async() => {
        const query = `SELECT [desc], name, date FROM bbs WHERE (((bbs.P_idx)=${Number(glovar.VideoDetail_P_idx)}));`
    ...
    const upreply = async() => {
        const query = `INSERT INTO bbs (email, name, [desc], Reply_chk, P_idx) VALUES('${String(glovar.logininfo.email)}', '${String(glovar.logininfo.name)}', '${String(reply.value)}', ${Number(1)}, ${Number(glovar.VideoDetail_P_idx)})`
        const result = await MSSQL.executeQuery(query);
        setreplys(result)

...
export default function Mypage () {
  ...
  const calldata = async() => {
    ...
    const query = `SELECT [사용권동의여부] as Agreement, [Product], [교육과정] as Curriculum, [StartDate] as SigninDate, [가입일자] as SubscribeStart, [종료일자] as SubscribeEnd, [비밀번호힌트] as PasswdHint, [상품구분] as ProductClass, [push] FROM [MSysMember] where [email]='${glovar.logininfo.email}' AND [사용자명]='${glovar.logininfo.name}'`
  ...
  const SubsDate = () => {
    if (MyData.SubscribeStart === undefined) {
      console.log("Mypage_SubsDate()_SubscribeStart === undefined ======================");
      console.log("SubscribeStart: ");
      console.log(MyData.SubscribeStart);
      console.log("================================================================================");
      return ""
    } else if (MyData.SubscribeEnd === undefined) {
      console.log("Mypage_SubsDate()_SubscribeEnd === undefined ======================");
      console.log("SubscribeEnd: ");
      console.log(MyData.SubscribeEnd);
      console.log("================================================================================");
      return ""
    } else if (MyData.SubscribeStart >= MyData.SubscribeEnd) {
      console.log("Mypage_SubsDate()_SubscribeStart >= SubscribeEnd ======================");
    ...
    <ScrollView style={styles.Container}>
      ...
      <View style={styles.subscribeBox}>
        <View style={styles.subsInnerBox}>
          ...
          <TouchableOpacity style={styles.BtnSubsManagment}>
            <Text style={styles.txtSubsManagment}>구독 관리<Icon style={styles.etcIcon2} name="chevron-right" size={13.5} color="#bbb" /></Text>
          </TouchableOpacity>
        </View>
        <View style={styles.txtSubsDateBox}>
          <Text style={styles.txtSubsDate}>구독 기간</Text>
          <Text style={styles.txtSubsDate}>{SubsStartDate()} ~ {SubsEndDate()}</Text>
        </View>
      ...
      <View style={styles.etcBox}>
        <TouchableOpacity style={styles.BtnEtcBox}>
            <Icon style={styles.etcIcon1} name="settings" size={20} color="#000" />
            <Text style={styles.txtEtcDesc}>상품 구분</Text>
            <Icon style={styles.etcIcon2} name="chevron-right" size={20} color="#000" />
        </TouchableOpacity>
      </View>
      <View style={styles.etcBox}>
        <TouchableOpacity style={styles.BtnEtcBox}>

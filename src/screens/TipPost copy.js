...
const TipPost = ({ navigation }) => {
  ...
  const writedone = async() => {
    if (title.value === "") {
      ...
      const query1 = `INSERT INTO Tip (category, title, [desc], email, name) VALUES('${String(category)}', '${String(title)}', '${String(desc)}', '${String(glovar.logininfo.email)}', '${String(glovar.logininfo.name)}')`
      ...
      navigation.navigate('TipDetail', {item: glovar.lastTipDetail[0]})
      glovar.P_idx = glovar.lastTipDetail[0].idx

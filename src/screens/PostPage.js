...
import {createP_idx} from '../redux/modules/allStore';
...
export default function PostPage({navigation}) {
  ...
  const insertQuery = async () => {
    const query1 = `INSERT INTO bbs (category, title, [desc], email, name, Thumbnail, Media1) VALUES('${String(
      category.value,
  ...
  const selectQuery = async () => {
    const query2 = `SELECT TOP 1 category, date, [desc], idx, thumbnail, Media1, title, name, Like_cnt, Reply_cnt, View_cnt FROM bbs WHERE [email]='${String(
      logininfo.email,
    ...
    lastDetail = result2[0];

    navigation.goBack();
    navigation.navigate('DetailPage', {item: lastDetail});
    // P_idx = glovar.lastDetail.idx;
  ...
  const onSelectedImage = async (image) => {
    ...
    if (Array.isArray(image) === true) {
      if (image.length > 8) {
        ...
        console.log("PostPage_if (image.length > 7) ==========");
        console.log("이미지가 8개를 초과했습니다! 8개 이하로 선택해 주세요.");
  ...
  const prptImg = (uris) => {
    ...
    if (uris.length >= 1) {
      ...
      if (images.length > 8) {
        ...
        // console.log("========================================");
        // console.log("PostPage_prptImg_if uris.length >= 1_if images.length > 8");
        // console.log("images.length: " + images.length);
        // console.log("========================================");

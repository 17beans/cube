...
export default function PostPageEdit ({ navigation }) {
  ...
  const [title, settitle] = useState({ value: glovar.lastDetail.title, error: '' })
  const [category, setcategory] = useState({ value: glovar.lastDetail.category, error: '' })
  const [desc, setdesc] = useState({ value: glovar.lastDetail.desc, error: '' })
  ...
  const updateQuery = async() => {
    ...
    const query = `UPDATE bbs SET Title='${title.value}', Category='${category.value}', [Desc]='${desc.value}' WHERE Idx=${glovar.lastDetail.idx}`
  ...
  const selectQuery = async() => {
    // const query = `SELECT TOP 1 category, date, [desc], idx, thumbnail, title, name, Like_cnt, Reply_cnt, View_cnt FROM bbs WHERE [email]='${String(glovar.logininfo.email)}' AND [name]='${glovar.logininfo.name}' AND [Reply_chk]=0 ORDER BY idx desc`
    const query = `SELECT category, date, [desc], idx, thumbnail, Media1, title, name, Like_cnt, Reply_cnt, View_cnt FROM bbs WHERE Idx=${glovar.lastDetail.idx}`
    ...
    console.log("glovar.lastDetail: " + JSON.stringify(glovar.lastDetail));
    console.log("========================================");

    navigation.goBack()
    navigation.goBack()
    navigation.navigate('DetailPage', {item: glovar.lastDetail})
    // navigation.replace('DetailPage', {item:JSON.stringify(glovar.lastDetail)})
  ...
  useEffect(()=>{
    // settitle({value:glovar.lastDetail.title})
    // setcategory({value: glovar.lastDetail.category})
    // setdesc({value: glovar.lastDetail.desc})
      ...
      <ScrollView style={styles.test}>
        <View style={styles.title}>
          <TextInput
            ...
            returnKeyType="next"
            value={title.value}
            // value={glovar.lastDetail.title}    // 값 변경이 불가해짐
        ...
        <View style={styles.desccontainer}>
          <TextInput
            ...
            returnKeyType="next"
            value={desc.value}
            // value={glovar.lastDetail.desc}    // 값 변경이 불가해짐

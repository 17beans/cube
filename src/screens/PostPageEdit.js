...
export default function PostPageEdit({navigation}) {
  ...
  const [title, settitle] = useState({
    value: lastDetail.title,
    error: '',
  });
  const [category, setcategory] = useState({
    value: lastDetail.category,
    error: '',
  });
  const [desc, setdesc] = useState({value: lastDetail.desc, error: ''});
  ...
  const writedone = async () => {
    if (title.value === '') {
      ...
      images.map((content, i) => {
        ...
        // console.log("========================================");
        // console.log('PostPage_writedone()_else_images.map');
        // console.log("========================================");
  ...
  const updateQuery = async () => {
    const query = `UPDATE bbs SET Title='${title.value}', Category='${category.value}', [Desc]='${desc.value}' WHERE Idx=${lastDetail.idx}`;

    console.log('PostPageEdit_UPDATE Query ==================');
    console.log(query);
    console.log('========================================');
  ...
  const selectQuery = async () => {
    // const query = `SELECT TOP 1 category, date, [desc], idx, thumbnail, title, name, Like_cnt, Reply_cnt, View_cnt FROM bbs WHERE [email]='${String(logininfo.email)}' AND [name]='${logininfo.name}' AND [Reply_chk]=0 ORDER BY idx desc`
    const query = `SELECT category, date, [desc], idx, thumbnail, Media1, title, name, Like_cnt, Reply_cnt, View_cnt FROM bbs WHERE Idx=${lastDetail.idx}`;
    const result = await MSSQL.executeQuery(query);

    // glovar.lastDetail = result[0];
    dispatch(createLastDetail(result[0]));

    console.log('========================================');
    console.log('PostPageEdit_selectQuery()');
    console.log('lastDetail: ' + JSON.stringify(lastDetail));
    console.log('========================================');

    navigation.goBack();
    navigation.goBack();
    navigation.navigate('DetailPage', {item: lastDetail});
    // navigation.replace('DetailPage', {item:JSON.stringify(lastDetail)})
    // navigation.replace('SideBar')
    // glovar.P_idx = lastDetail.idx;
    dispatch(createP_idx(lastDetail.idx));
  };
  ...
  const CatePicker = () => {
    return (
      <RNPickerSelect
        ...
        // value={glovar.lastDetail.category}    // 값 변경이 불가해짐
        placeholder={{
          label: '카테고리',
          value: null,
          color: 'gray',
  ...
  const onSelectedImage = async (image) => {
    ...
    if (Array.isArray(image) === true) {
      if (image.length > 8) {
        console.log('PostPageEdit_if (image.length > 8) ==========');
        ...
        console.log("PostPageEdit_if (image.length > 7) ==========");
        console.log("이미지가 8개를 초과했습니다! 8개 이하로 선택해 주세요.");
  ...
  const prptImg = (uris) => {
    ...
    if (uris.length >= 1) {
      ...
      if (images.length > 8) {
        ...
        // console.log("========================================");
        // console.log("PostPageEdit_prptImg_if uris.length >= 1_if images.length > 8");
        // console.log("images.length: " + images.length);
        // console.log("========================================");
  ...
  useEffect(() => {
    // settitle({value:lastDetail.title})
    // setcategory({value: lastDetail.category})
    // setdesc({value: lastDetail.desc})
      ...
      <ScrollView style={styles.test}>
        <View style={styles.title}>
          <TextInput
            ...
            returnKeyType="next"
            value={title.value}
            // value={lastDetail.title}    // 값 변경이 불가해짐
        ...
        <View style={styles.desccontainer}>
          <TextInput
            ...
            returnKeyType="next"
            value={desc.value}
            // value={lastDetail.desc}    // 값 변경이 불가해짐

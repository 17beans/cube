...
export default function QnAPostEdit({navigation}) {
  ...
  const [title, settitle] = useState({value: lastQnADetail.title, error: ''});
  const [category, setcategory] = useState({
    value: lastQnADetail.category,
    error: '',
  });
  const [desc, setdesc] = useState({value: lastQnADetail.desc, error: ''});
  ...
  const updateQuery = async () => {
    const query = `UPDATE QnA SET Title='${title.value}', Category='${category.value}', [Desc]='${desc.value}' WHERE Idx=${lastQnADetail.idx}`;

    console.log('QnAPostEdit_UPDATE Query ==================');
    console.log(query);
    console.log('========================================');
  ...
  const selectQuery = async () => {
    // const query = `SELECT TOP 1 category, date, [desc], idx, thumbnail, title, name, Like_cnt, Reply_cnt, View_cnt FROM QnA WHERE [email]='${String(logininfo.email)}' AND [name]='${logininfo.name}' AND [Reply_chk]=0 ORDER BY idx desc`
    const query = `SELECT category, date, [desc], idx, thumbnail, Media1, title, name, Like_cnt, Reply_cnt, View_cnt FROM QnA WHERE Idx=${lastQnADetail.idx}`;
    const result = await MSSQL.executeQuery(query);

    // glovar.lastQnADetail = result[0]
    dispatch(createLastQnADetail(result[0]));

    console.log('========================================');
    console.log('QnAPostEdit_selectQuery()');
    console.log('lastQnADetail: ' + JSON.stringify(lastQnADetail));
    console.log('========================================');

    navigation.goBack();
    navigation.goBack();
    navigation.navigate('QnADetail', {item: lastQnADetail});
    // navigation.replace('QnADetail', {item:JSON.stringify(lastQnADetail)})
    // navigation.replace('SideBar')
    // glovar.QnAP_idx = lastQnADetail.idx;
    dispatch(createQnAP_idx(lastQnADetail.idx));
  };
  ...
  const CatePicker = () => {
    return (
      <RNPickerSelect
        ...
        // value={glovar.lastQnADetail.category}    // 값 변경이 불가해짐
        placeholder={{
          label: '카테고리',
          value: null,
          color: 'gray',
  ...
  const onSelectedImage = async (image) => {
    ...
    if (Array.isArray(image) === true) {
      if (image.length > 8) {
        console.log('QnAPostEdit_if (image.length > 8) ==========');
        ...
        console.log("QnAPostEdit_if (image.length > 7) ==========");
        console.log("이미지가 8개를 초과했습니다! 8개 이하로 선택해 주세요.");
  ...
  const prptImg = (uris) => {
    ...
    if (uris.length >= 1) {
      ...
      if (images.length > 8) {
        ...
        // console.log("========================================");
        // console.log("QnAPostEdit_prptImg_if uris.length >= 1_if images.length > 8");
        // console.log("images.length: " + images.length);
        // console.log("========================================");
  ...
  useEffect(() => {
    // settitle({value:lastQnADetail.title})
    // setcategory({value: lastQnADetail.category})
    // setdesc({value: lastQnADetail.desc})
      ...
      <ScrollView style={styles.test}>
        <View style={styles.title}>
          <TextInput
            ...
            returnKeyType="next"
            value={title.value}
            // value={lastQnADetail.title}    // 값 변경이 불가해짐
        ...
        <View style={styles.desccontainer}>
          <TextInput
            ...
            returnKeyType="next"
            value={desc.value}
            // value={lastQnADetail.desc}    // 값 변경이 불가해짐

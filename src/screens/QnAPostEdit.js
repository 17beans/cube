...
export default function QnAPostEdit ({ navigation }) {
  ...
  const [title, settitle] = useState({ value: glovar.lastQnADetail.title, error: '' })
  const [category, setcategory] = useState({ value: glovar.lastQnADetail.category, error: '' })
  const [desc, setdesc] = useState({ value: glovar.lastQnADetail.desc, error: '' })
  ...
  const updateQuery = async() => {
    ...
    const query = `UPDATE QnA SET Title='${title.value}', Category='${category.value}', [Desc]='${desc.value}' WHERE Idx=${glovar.lastQnADetail.idx}`
  ...
  const selectQuery = async() => {
    // const query = `SELECT TOP 1 category, date, [desc], idx, thumbnail, title, name, Like_cnt, Reply_cnt, View_cnt FROM QnA WHERE [email]='${String(glovar.logininfo.email)}' AND [name]='${glovar.logininfo.name}' AND [Reply_chk]=0 ORDER BY idx desc`
    const query = `SELECT category, date, [desc], idx, thumbnail, Media1, title, name, Like_cnt, Reply_cnt, View_cnt FROM QnA WHERE Idx=${glovar.lastQnADetail.idx}`
    ...
    console.log("glovar.lastQnADetail: " + JSON.stringify(glovar.lastQnADetail));
    console.log("========================================");

    navigation.goBack()
    navigation.goBack()
    navigation.navigate('QnADetail', {item: glovar.lastQnADetail})
    // navigation.replace('QnADetail', {item:JSON.stringify(glovar.lastQnADetail)})
  ...
  useEffect(()=>{
    // settitle({value:glovar.lastQnADetail.title})
    // setcategory({value: glovar.lastQnADetail.category})
    // setdesc({value: glovar.lastQnADetail.desc})
      ...
      <ScrollView style={styles.test}>
        <View style={styles.title}>
          <TextInput
            ...
            returnKeyType="next"
            value={title.value}
            // value={glovar.lastQnADetail.title}    // 값 변경이 불가해짐
        ...
        <View style={styles.desccontainer}>
          <TextInput
            ...
            returnKeyType="next"
            value={desc.value}
            // value={glovar.lastQnADetail.desc}    // 값 변경이 불가해짐

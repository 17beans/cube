...
export default function TipPostEdit({navigation}) {
  ...
  const [title, settitle] = useState({
    value: lastTipDetail.title,
    error: '',
  });
  const [category, setcategory] = useState({
    value: lastTipDetail.category,
    error: '',
  });
  const [desc, setdesc] = useState({
    value: lastTipDetail.desc,
    error: '',
  });
  ...
  const writedone = async () => {
    if (title.value === '') {
      ...
      images.map((content, i) => {
        ...
        insImgUris = trim([
          ...insImgUris,
          'http://access.co.kr/appimg/upload/' + filename,
        ]);

        images = [];

        // console.log("========================================");
        // console.log('TipPost_writedone()_else_images.map');
        // console.log("========================================");
  ...
  const updateQuery = async () => {
    const query = `UPDATE Tip SET Title='${title.value}', Category='${category.value}', [Desc]='${desc.value}' WHERE Idx=${lastTipDetail.idx}`;

    console.log('TipPostEdit_UPDATE Query ==================');
    console.log(query);
    console.log('========================================');
  ...
  const selectQuery = async () => {
    // const query = `SELECT TOP 1 category, date, [desc], idx, thumbnail, title, name, Like_cnt, Reply_cnt, View_cnt FROM Tip WHERE [email]='${String(logininfo.email)}' AND [name]='${logininfo.name}' AND [Reply_chk]=0 ORDER BY idx desc`
    const query = `SELECT category, date, [desc], idx, thumbnail, Media1, title, name, Like_cnt, Reply_cnt, View_cnt FROM Tip WHERE Idx=${lastTipDetail.idx}`;
    const result = await MSSQL.executeQuery(query);

    // glovar.lastTipDetail = result[0];
    dispatch(createLastTipDetail(result[0]));

    console.log('========================================');
    console.log('TipPostEdit_selectQuery()');
    console.log('lastTipDetail: ' + JSON.stringify(lastTipDetail));
    console.log('========================================');

    navigation.goBack();
    navigation.goBack();
    navigation.navigate('TipDetail', {item: lastTipDetail});
    // navigation.replace('TipDetail', {item:JSON.stringify(glovar.lastTipDetail)})
    // navigation.replace('SideBar')
    // glovar.TipP_idx = lastTipDetail.idx;
    dispatch(createTipP_idx(lastTipDetail.idx));
  };
  ...
  const CatePicker = () => {
    return (
      <RNPickerSelect
        ...
        // value={glovar.lastTipDetail.category}    // 값 변경이 불가해짐
        placeholder={{
          label: '카테고리',
          value: null,
          color: 'gray',
  ...
  const onSelectedImage = async (image) => {
    ...
    if (Array.isArray(image) === true) {
      if (image.length > 8) {
        console.log('TipPostEdit_if (image.length > 8) ==========');
        console.log('이미지가 8개를 초과했습니다! 8개 이하로 선택해 주세요.');
        Alert.alert(
          '',
          '이미지가 8개를 초과했습니다!\n8개 이하로 선택해 주세요.',
        );
      } /*else if (images.length > 7) {    // 첫 번째에 선택된 사진들은 카운트가 안되는 듯. 수정 필요. prptImg()에서 배열 갯수 카운트 가능하기에 prptImg()에서 처리
        console.log("TipPostEdit_if (image.length > 7) ==========");
        console.log("이미지가 8개를 초과했습니다! 8개 이하로 선택해 주세요.");
  ...
  const prptImg = (uris) => {
    ...
    if (uris.length >= 1) {
      ...
      if (images.length > 8) {
        // extrectURIs()에서 동작하지 않아서 여기서 처리
        // console.log("========================================");
        // console.log("TipPostEdit_prptImg_if uris.length >= 1_if images.length > 8");
        // console.log("images.length: " + images.length);
        // console.log("========================================");
  ...
  useEffect(() => {
    // settitle({value:lastTipDetail.title})
    // setcategory({value: lastTipDetail.category})
    // setdesc({value: lastTipDetail.desc})
      ...
      <ScrollView style={styles.test}>
        <View style={styles.title}>
          <TextInput
            ...
            returnKeyType="next"
            value={title.value}
            // value={lastTipDetail.title}    // 값 변경이 불가해짐
        ...
        <View style={styles.mediacontainer}>
          ...
          <View style={styles.mediabtn}>
            <TouchableOpacity
              ...
              <Text style={styles.btnuploadtxt}>
                선택{'\n'}
                {images.length}/8
              </Text>
        ...
        <View style={styles.desccontainer}>
          <TextInput
            ...
            returnKeyType="next"
            value={desc.value}
            // value={lastTipDetail.desc}    // 값 변경이 불가해짐

...
export default function TipPostEdit ({ navigation }) {
  ...
  const [title, settitle] = useState({ value: glovar.lastTipDetail.title, error: '' })
  const [category, setcategory] = useState({ value: glovar.lastTipDetail.category, error: '' })
  const [desc, setdesc] = useState({ value: glovar.lastTipDetail.desc, error: '' })
  ...
  const updateQuery = async() => {
    ...
    const query = `UPDATE Tip SET Title='${title.value}', Category='${category.value}', [Desc]='${desc.value}' WHERE Idx=${glovar.lastTipDetail.idx}`
  ...
  const selectQuery = async() => {
    // const query = `SELECT TOP 1 category, date, [desc], idx, thumbnail, title, name, Like_cnt, Reply_cnt, View_cnt FROM Tip WHERE [email]='${String(glovar.logininfo.email)}' AND [name]='${glovar.logininfo.name}' AND [Reply_chk]=0 ORDER BY idx desc`
    const query = `SELECT category, date, [desc], idx, thumbnail, Media1, title, name, Like_cnt, Reply_cnt, View_cnt FROM Tip WHERE Idx=${glovar.lastTipDetail.idx}`
    ...
    console.log("glovar.lastTipDetail: " + JSON.stringify(glovar.lastTipDetail));
    console.log("========================================");

    navigation.goBack()
    navigation.goBack()
    navigation.navigate('TipDetail', {item: glovar.lastTipDetail})
    // navigation.replace('TipDetail', {item:JSON.stringify(glovar.lastTipDetail)})
    // navigation.replace('SideBar')
    glovar.TipP_idx = glovar.lastTipDetail.idx
  }
  ...
  useEffect(()=>{
    // settitle({value:glovar.lastTipDetail.title})
    // setcategory({value: glovar.lastTipDetail.category})
    // setdesc({value: glovar.lastTipDetail.desc})
      ...
      <ScrollView style={styles.test}>
        <View style={styles.title}>
          <TextInput
            ...
            returnKeyType="next"
            value={title.value}
            // value={glovar.lastTipDetail.title}    // 값 변경이 불가해짐
        ...
        <View style={styles.desccontainer}>
          <TextInput
            ...
            returnKeyType="next"
            value={desc.value}
            // value={glovar.lastTipDetail.desc}    // 값 변경이 불가해짐

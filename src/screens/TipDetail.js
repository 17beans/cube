...
export default function TipDetail({navigation, route}) {
  ...
  const callreply = async () => {
    ...
    const query = `SELECT Idx, [desc], name, email, date, thumbnail FROM Tip WHERE (((Tip.P_idx)=${Number(
      TipP_idx,
    )})) ORDER BY Idx;`;
    const result = await MSSQL.executeQuery(query);

    // const query2 = `SELECT Idx, Count(*) AS Reply_cnt2 FROM Tip GROUP BY Idx, Reply_chk, P_idx HAVING Reply_chk=1 AND P_idx=${Number(TipP_idx)};`
    ...
    // const query3 = `UPDATE Tip SET Reply_cnt=${Number(result2.length)} WHERE [Idx] = ${TipP_idx}`
    // const result3 = MSSQL.executeQuery(query3);

    const query2 = `SELECT Count(*) AS Reply_cnt2 FROM Tip WHERE P_idx=${TipP_idx}`;
    const result2 = await MSSQL.executeQuery(query2);
    ...
    // const query2 = `SELECT Count(*) AS Reply_cnt2 FROM Tip GROUP BY Reply_chk, P_idx HAVING Reply_chk=1 AND P_idx=${TipP_idx};`
    // const result2 = await MSSQL.executeQuery(query2);
  ...
  const upreply = async () => {
    if (reply === '') {
      ...
      const query = `INSERT INTO Tip (email, name, [desc], Reply_chk, P_idx) VALUES('${String(
        logininfo.email,
      )}', '${String(logininfo.name)}', '${String(reply)}', 1, ${Number(
        TipP_idx,
      )})`;

      const query2 = `UPDATE Tip SET Reply_cnt=Reply_cnt+1 WHERE Idx=${lastTipDetail.idx}`;

      // navigation.goBack()
      // navigation.navigate('TipDetail', {item:lastTipDetail})
      navigation.replace('TipDetail', {item: lastTipDetail});
  ...
  useEffect(() => {
    ...
    // console.log("lastTipDetail.thumbnail: " + lastTipDetail.thumbnail);
    // console.log("========================================");
  }, []);

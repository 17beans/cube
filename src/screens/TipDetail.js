...
import BtnLikeTip from '../components/BtnLikeTip';
import glovar from '../components/glovar'
// import ImageCollection from '../components/ImageCollection'
import ImageSliderTip from '../components/ImageSliderTip'
import BtnDelReplyTip from '../components/BtnDelReplyTip'
...
export default function TipDetail({navigation, route}) {
    ...
    const callreply = async() => {
        ...
        const query = `SELECT Idx, [desc], name, email, date, thumbnail FROM Tip WHERE (((Tip.P_idx)=${Number(glovar.TipP_idx)})) ORDER BY Idx;`
        const result = await MSSQL.executeQuery(query);

        // const query2 = `SELECT Idx, Count(*) AS Reply_cnt2 FROM Tip GROUP BY Idx, Reply_chk, P_idx HAVING Reply_chk=1 AND P_idx=${Number(glovar.TipP_idx)};`
        // const result2 = await MSSQL.executeQuery(query2);
        
        // // console.log("TipDetail_query2 ======================");
        // // console.log("query2: \n" + query2);
        // // console.log("========================================");

        // const query3 = `UPDATE Tip SET Reply_cnt=${Number(result2.length)} WHERE [Idx] = ${glovar.TipP_idx}`
        // const result3 = MSSQL.executeQuery(query3);

        const query2 = `SELECT Count(*) AS Reply_cnt2 FROM Tip WHERE P_idx=${glovar.TipP_idx}`
        const result2 = await MSSQL.executeQuery(query2);
        ...
        console.log("========================================");
        console.log("TipDetail_callreply()");
        console.log("========================================");
        console.log("result(댓글들) " + JSON.stringify(result));
        // console.log("glovar.TipP_idx: " + glovar.TipP_idx);
        // console.log("result2 " + JSON.stringify(result2[0].Reply_cnt2));
        // console.log("newReplyCnt: " + newReplyCnt);    // TipDetail가 실행될 때 callreply()에서 별도로 불러온 댓글 갯수
        ...
        // const query2 = `SELECT Count(*) AS Reply_cnt2 FROM Tip GROUP BY Reply_chk, P_idx HAVING Reply_chk=1 AND P_idx=${glovar.TipP_idx};`
        // const result2 = await MSSQL.executeQuery(query2);
    ...
    const upreply = async() => {
        if (reply === "") {
            ...
            const query = `INSERT INTO Tip (email, name, [desc], Reply_chk, P_idx) VALUES('${String(glovar.logininfo.email)}', '${String(glovar.logininfo.name)}', '${String(reply)}', 1, ${Number(glovar.TipP_idx)})`

            const query2 = `UPDATE Tip SET Reply_cnt=Reply_cnt+1 WHERE Idx=${glovar.lastTipDetail.idx}`
            
            // navigation.goBack()
            // navigation.navigate('TipDetail', {item:glovar.lastTipDetail})
            navigation.replace('TipDetail', {item:glovar.lastTipDetail})

            console.log("TipDetail_upreply =====================");
    ...
    useEffect(()=>{
        ...
        setdata(item)
        callreply()
        // console.log("TipDetail_useEffect ===================");
        // console.log("glovar.lastTipDetail.thumbnail: " + glovar.lastTipDetail.thumbnail);
        // console.log("========================================");
    },[])
                ...
                <View>
                    <View style={styles.cntReplyLike}>
                        <View style={styles.replycnt}>
                            ...
                            {/** 보다 정확한 댓글 갯수 불러오기를 위해 TipDetail를 실행할 때 callreply() 부분에서 댓글 갯수를 newReplyCnt라는 변수에 따로 불러옴 */}
                        </View>
                        
                        <View style={styles.cnt2}>
                            <BtnLikeTip style={styles.BtnLike} likecnt={likecnt} />
                        </View>
                    </View>
                </View>
                    ...
                    renderItem={({item}) => (
                        <View style={styles.replyscontainer}>
                            ...
                            <View style={styles.dateNbtns}>
                                ...
                                <BtnDelReplyTip navigation={navigation} email1={item.email} email2={glovar.logininfo.email} idx={item.Idx} />
                            </View>
                        </View>
                    )}

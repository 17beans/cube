...
export default function QnAPost({navigation}) {
  ...
  const insertQuery = async () => {
    const query1 = `INSERT INTO QnA (category, title, [desc], email, name, Thumbnail, Media1) VALUES('${String(
      category.value,
  ...
  const selectQuery = async () => {
    const query2 = `SELECT TOP 1 category, date, [desc], idx, thumbnail, title, name, Like_cnt, Reply_cnt, View_cnt FROM QnA WHERE [email]='${String(
      logininfo.email,
    ...
    dispatch(createLastQnADetail(result2[0]));

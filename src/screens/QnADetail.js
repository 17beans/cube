...
export default function QnADetail({navigation, route}) {
    ...
    const callreply = async() => {
        ...
        const query = `SELECT Idx, [desc], name, email, date, thumbnail FROM QnA WHERE (((QnA.P_idx)=${Number(glovar.QnAP_idx)})) ORDER BY Idx;`
        const result = await MSSQL.executeQuery(query);

        // const query2 = `SELECT Idx, Count(*) AS Reply_cnt2 FROM QnA GROUP BY Idx, Reply_chk, P_idx HAVING Reply_chk=1 AND P_idx=${Number(glovar.QnAP_idx)};`
        ...
        // const query3 = `UPDATE QnA SET Reply_cnt=${Number(result2.length)} WHERE [Idx] = ${glovar.QnAP_idx}`
        // const result3 = MSSQL.executeQuery(query3);

        const query2 = `SELECT Count(*) AS Reply_cnt2 FROM QnA WHERE P_idx=${glovar.QnAP_idx}`
        const result2 = await MSSQL.executeQuery(query2);
        ...
        // console.log("glovar.QnAP_idx: " + glovar.QnAP_idx);
        ...
        // const query2 = `SELECT Count(*) AS Reply_cnt2 FROM QnA GROUP BY Reply_chk, P_idx HAVING Reply_chk=1 AND P_idx=${glovar.QnAP_idx};`
        // const result2 = await MSSQL.executeQuery(query2);
    ...
    const upreply = async() => {
        if (reply === "") {
            ...
            const query = `INSERT INTO QnA (email, name, [desc], Reply_chk, P_idx) VALUES('${String(glovar.logininfo.email)}', '${String(glovar.logininfo.name)}', '${String(reply)}', 1, ${Number(glovar.QnAP_idx)})`

            const query2 = `UPDATE QnA SET Reply_cnt=Reply_cnt+1 WHERE Idx=${glovar.lastQnADetail.idx}`
            
            // navigation.goBack()
            // navigation.navigate('QnADetail', {item:glovar.lastQnADetail})
            navigation.replace('QnADetail', {item:glovar.lastQnADetail})
    ...
    useEffect(()=>{
        ...
        // console.log("glovar.lastQnADetail.thumbnail: " + glovar.lastQnADetail.thumbnail);
        // console.log("========================================");
    },[])
                    ...
                    renderItem={({item}) => (
                        <View style={styles.replyscontainer}>
                            ...
                            <View style={styles.dateNbtns}>
                                ...
                                <BtnDelReplyQnA navigation={navigation} email1={item.email} email2={glovar.logininfo.email} idx={item.Idx} />
                            </View>
                        </View>
                    )}

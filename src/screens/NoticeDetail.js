...
import ImageCollectionNotice from '../components/ImageCollectionNotice';
...
export default function NoticeDetail({navigation, route}) {
  const NoticeDetail_P_idx = useSelector(
    (state) => state.allStore.NoticeDetail_P_idx,
  );
  ...
  const callreply = async () => {
    ...
    const query2 = `SELECT Idx, Count(*) AS Reply_cnt2 FROM bbs GROUP BY Idx, Reply_chk, P_idx HAVING Reply_chk=1 AND P_idx=${Number(
      NoticeDetail_P_idx,
    )};`;
    const result2 = await MSSQL.executeQuery(query2);
    console.log('NoticeDetail_query2 ======================');
    console.log('query2: \n' + query2);
    console.log('========================================');

    const query3 = `UPDATE bbs SET Reply_cnt=${Number(
      result2.length,
    )} WHERE [Idx] = ${NoticeDetail_P_idx}`;
    const result3 = MSSQL.executeQuery(query3);

    setreplys(result);
    console.log('NoticeDetail_callreply() =================');
    console.log(result);
    console.log('========================================');
    // const closed = MSSQL.close();

    // const query2 = `SELECT Idx, Count(*) AS Reply_cnt2 FROM bbs GROUP BY Idx, Reply_chk, P_idx HAVING Reply_chk=1 AND P_idx=${Number(NoticeDetail_P_idx)};`
    // const result2 = await MSSQL.executeQuery(query2);
    // console.log("query2: \n" + query2);

    // glovar.Reply_cnt = result2.length

    // const query2 = `SELECT Count(*) AS Reply_cnt2 FROM bbs GROUP BY Reply_chk, P_idx HAVING Reply_chk=1 AND P_idx=${NoticeDetail_P_idx};`
    // const result2 = await MSSQL.executeQuery(query2);
  ...
  //         const query = `INSERT INTO bbs (email, name, [desc], Reply_chk, P_idx) VALUES('${String(logininfo.email)}', '${String(logininfo.name)}', '${String(reply)}', ${Number(1)}, ${Number(NoticeDetail_P_idx)})`
  //         navigation.goBack()
  //         navigation.navigate('NoticeDetail', {item:lastDetail})
  //         console.log("NoticeDetail_upreply =====================");
  ...
  useEffect(() => {
    ...
    setdata(item);
    callreply();
    // console.log("NoticeDetail_useEffect ===================");

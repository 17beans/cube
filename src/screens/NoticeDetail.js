...
export default function NoticeDetail({navigation, route}) {
    ...
    const callreply = async() => {
        const query = `SELECT [desc], name, date FROM bbs WHERE (((bbs.P_idx)=${Number(glovar.NoticeDetail_P_idx)}));`
        const result = await MSSQL.executeQuery(query);

        const query2 = `SELECT Idx, Count(*) AS Reply_cnt2 FROM bbs GROUP BY Idx, Reply_chk, P_idx HAVING Reply_chk=1 AND P_idx=${Number(glovar.NoticeDetail_P_idx)};`
        const result2 = await MSSQL.executeQuery(query2);
        console.log("NoticeDetail_query2 ======================");
        console.log("query2: \n" + query2);
        console.log("========================================");

        const query3 = `UPDATE bbs SET Reply_cnt=${Number(result2.length)} WHERE [Idx] = ${glovar.NoticeDetail_P_idx}`
        ...
        // const query2 = `SELECT Count(*) AS Reply_cnt2 FROM bbs GROUP BY Reply_chk, P_idx HAVING Reply_chk=1 AND P_idx=${glovar.NoticeDetail_P_idx};`
        // const result2 = await MSSQL.executeQuery(query2);
    ...
    //         const query = `INSERT INTO bbs (email, name, [desc], Reply_chk, P_idx) VALUES('${String(glovar.logininfo.email)}', '${String(glovar.logininfo.name)}', '${String(reply)}', ${Number(1)}, ${Number(glovar.NoticeDetail_P_idx)})`
    //         navigation.goBack()
    //         navigation.navigate('NoticeDetail', {item:glovar.lastDetail})
    //         console.log("NoticeDetail_upreply =====================");
    //         console.log(glovar.logininfo.email);
    //         console.log(glovar.logininfo.name);

...
export default function TipPost({navigation}) {
  ...
  const insertQuery = async () => {
    const query1 = `INSERT INTO Tip (category, title, [desc], email, name, Thumbnail, Media1) VALUES('${String(
      category.value,
    )}', '${String(title.value)}', '${String(desc.value)}', '${String(
      logininfo.email,
    )}', '${String(logininfo.name)}', '${insImgUris[0]}', '${insImgUris}')`;

    const result = MSSQL.executeQuery(query1);

    console.log('========================================');
    console.log('TipPost_Insert Query');
    console.log('========================================');
    console.log(query1);
    console.log('========================================');
  };
  ...
  const selectQuery = async () => {
    const query2 = `SELECT TOP 1 category, date, [desc], idx, thumbnail, title, name, Like_cnt, Reply_cnt, View_cnt FROM Tip WHERE [email]='${String(
      logininfo.email,
    )}' AND [name]='${logininfo.name}' AND [Reply_chk]=0 ORDER BY idx desc`;

    const result2 = await MSSQL.executeQuery(query2);

    console.log('========================================');
    console.log('TipPost_Select Query');
    console.log('========================================');
    console.log(query2);
    console.log(result2);
    console.log('========================================');

    // glovar.lastTipDetail = result2[0];
  ...
  const onSelectedImage = async (image) => {
    ...
    if (Array.isArray(image) === true) {
      if (image.length > 8) {
        console.log('========================================');
        console.log('TipPost_if (image.length > 8)');
        console.log('========================================');
        ...
        console.log("TipPost_if (image.length > 7) ==========");
        console.log("이미지가 8개를 초과했습니다! 8개 이하로 선택해 주세요.");
  ...
  const prptImg = (uris) => {
    ...
    if (uris.length >= 1) {
      ...
      if (images.length > 8) {
        ...
        // console.log("========================================");
        // console.log("TipPost_prptImg_if uris.length >= 1_if images.length > 8");
        // console.log("images.length: " + images.length);
        // console.log("========================================");

...
export default function TipPost ({ navigation }) {
  ...
  const insertQuery = async() => {
    const query1 = `INSERT INTO Tip (category, title, [desc], email, name, Thumbnail, Media1) VALUES('${String(category.value)}', '${String(title.value)}', '${String(desc.value)}', '${String(glovar.logininfo.email)}', '${String(glovar.logininfo.name)}', '${insImgUris[0]}', '${insImgUris}')`
  ...
  const selectQuery = async() => {
    const query2 = `SELECT TOP 1 category, date, [desc], idx, thumbnail, title, name, Like_cnt, Reply_cnt, View_cnt FROM Tip WHERE [email]='${String(glovar.logininfo.email)}' AND [name]='${glovar.logininfo.name}' AND [Reply_chk]=0 ORDER BY idx desc`

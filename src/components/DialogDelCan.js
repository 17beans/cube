...
export default function DialogDelCan({navigation}){
    ...
    const delRecord = async() => {
        if (glovar.logininfo.email === glovar.lastDetail.email) {
            console.log("========================================");
            console.log("BtnDel_delRecord()_if glovar.logininfo.email = glovar.lastDetail.email");
            console.log("글을 올린 사람과 로그인한 사람의 이메일이 일치합니다. 글을 삭제합니다.");
            console.log("========================================");
            navigation.replace('SideBar')
            // navigation.goBack()

            const query1 = `DELETE FROM bbs WHERE Idx=${glovar.lastDetail.idx};`
            const result1 = MSSQL.executeQuery(query1)
            const query2 = `DELETE FROM bbs WHERE P_idx=${glovar.lastDetail.idx};`
        ...
        console.log("glovar.lastDetail.idx: " + glovar.lastDetail.idx);
    ...
    // console.log("========================================");
    // console.log("BtnDel");
    // console.log(glovar.lastDetail.email);
    // console.log(glovar.logininfo.email);
    // console.log(glovar.lastDetail.idx);
    // console.log("========================================");
    ...
    if (glovar.lastDetail.email === glovar.logininfo.email) {
        return (
            ...
            <HiddenItem title="삭제" icon={<Icon name="delete" size={24} color="#000" style={{}} />} onPress={() => DialogDelCan()} style={{width:5, height:35}} />
        )
    }
}

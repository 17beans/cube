...
export default function BtnDelReply({navigation, email1, email2, idx}){
    
    console.log("========================================");
    console.log("BtnDelReply");
    console.log("========================================");
    console.log("email1");
    console.log(email1);
    console.log("email2");
    ...
    const delReply = async(idx) => {
        if (email1 === email2) {
            ...
            const query1 = `DELETE FROM bbs WHERE Idx=${idx};`
            const result1 = MSSQL.executeQuery(query1)
            const query2 = `UPDATE bbs SET Reply_cnt=Reply_cnt-1 WHERE Idx=${glovar.P_idx};`
